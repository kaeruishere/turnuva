...
function drawDetail(navigate) {
  ...
  const main = document.getElementById('main');
  if (!document.getElementById('detail-root')) main.innerHTML = `<div id="detail-root"></div>`;
        ...
        <tbody>
          ${sorted.map((p, i) => `
            <tr>
              ...
              <td style="color:${p.ag-p.yg>0?'var(--accent)':p.ag-p.yg<0?'var(--red)':'var(--text2)'}">
                ${p.ag-p.yg>0?'+':''}${p.ag-p.yg}
              </td>
    ...
    ${nextMatch ? `
      <div class="card" style="cursor:pointer" id="next-match-card">
        <div class="next-match">
          <div class="nm-team">${nextMatch.home}</div>
          <div class="nm-vs">VS</div>
          <div class="nm-team">${nextMatch.away}</div>
        </div>
    ...
    <div class="card">
      ${matches.map(m => `
        <div class="match-row ${m.done?'done-match':''}" data-mid="${m.id}">
          ...
          <div class="mr-team right">${m.home}</div>
          <div class="mr-score ${m.done?'':'pending'}">${m.done?`${m.hG} - ${m.aG}`:'· · ·'}</div>
          <div class="mr-team">${m.away}</div>
          <div style="width:16px;font-size:.7rem;color:var(--text3)">${m.done?'✓':''}</div>
        </div>
      `).join('')}
    </div>
  `;
  ...
  if (nextMatch) document.getElementById('next-match-card').onclick = () => openMatchModal(nextMatch.id);
  document.querySelectorAll('.match-row').forEach(row => {
    row.onclick = () => openMatchModal(row.dataset.mid);
  });
}
